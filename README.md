...
- [ ]  피그마 또는 와이어프레임 제작
- [ ]  필수 기능(비동기 데이터, 예외처리) 구현 범위 확정
- [ ]  GitHub 레포 생성 및 팀원 협업 환경 세팅
- [ ]  폴더 구조 설계 (/css, /scss, /js, /images 등)

#### 2일차 — HTML 구조 설계 & 공통 요소 구현
- [ ]  HTML 시맨틱 구조 작성 (header, nav, main, footer)
- [ ]  공통 헤더·푸터 마크업 및 스타일 작성
- [ ]  SCSS 변수·믹스인 정의 (색상, 폰트, 여백)
- [ ]  기본 reset.css 또는 normalize.css 적용

#### 3일차 — 메인페이지 콘텐츠 구현
- [ ]  Hero 배너, 추천 상품, 이벤트 영역 마크업
- [ ]  SASS로 반응형 그리드/레이아웃 적용
- [ ]  JavaScript로 첫 번째 비동기 데이터 렌더링 구현 (예: 상품 목록)
- [ ]  예외처리 기본 로직 추가 (데이터 불러오기 실패 시 메시지 출력)

#### 4일차 — 서브페이지별 기능 개발 & 데이터 연동
- [ ]  팀원별 담당 서브페이지 HTML/CSS 작성
- [ ]  데이터 필터링·정렬 기능 추가
- [ ]  예외처리 고도화 (네트워크 오류, 빈 데이터 처리)
- [ ]  GitHub에 각자 브랜치 병합 전 PR 생성

#### 5일차 — 통합, 버그 수정 & 배포 준비
- [ ]  메인 + 서브페이지 통합
- [ ]  반응형 디테일 조정 (모바일·태블릿·데스크탑)
...
```bash
git clone https://xoxoworld.github.io/EST_fe_13_2st_project
```

**2. 프로젝트 실행**

- 클론 완료된 폴더 내의 `index.html` 파일을 더블 클릭하여 기본 브라우저에서 즉각 실행할 수 있습니다.
- 또는, VS Code 개발 환경에서 **Live Server** 익스텐션을 활성화하여 실시간 리로드 서버(`http://localhost:5500`) 환경에서 매끄럽게 웹 테스팅을 수행할 수 있습니다.

---

## 향후 개선 사항

- [ ] **성능 최적화**: 상품 이미지에 webp 및 반응형 이미지를 적용해 로딩 속도 개선, Lighthouse 성능 점수 개선
- [ ] **사용자 경험(UX) 개선**: 더 많은 페이지의 기능 추가
- [ ] **데이터 관리 개선**: json 구조를 api 기반 데이터 구조로 전환, 상태 관리 로직 개선
- [ ] **유지보수성 향상**: 코드 분리 및 모듈화 강화
---
